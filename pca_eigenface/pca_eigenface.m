...
M=4  ;% number faces
%% mic1
mic1 = imread('mic_training.jpg','jpg');
mic1=rgb2gray(mic1);
mic1=imresize(mic1,[N N] );
figure(1),imshow(mic1,'Initialmagnification','fit');;title('mic_training')


% shelly1
shelly1 = imread('shelly_training.jpg','jpg');
shelly1=rgb2gray(shelly1);
shelly1=imresize(shelly1,[N N] );
figure(2),imshow(shelly1,'Initialmagnification','fit');;title('shelly_training')


%% linoy1
linoy1 = imread('linoy_training.jpg','jpg');
linoy1=rgb2gray(linoy1);
linoy1=imresize(linoy1,[N N] );
figure(3),imshow(linoy1,'Initialmagnification','fit');;title('linoy_training')


%% libi1
libi1 = imread('libi_training.jpg','jpg');
libi1=rgb2gray(libi1);
libi1=imresize(libi1,[N N] );
figure(4),imshow(libi1,'Initialmagnification','fit');;title('libi_training')

%% extra face
extraface1 = imread('extraface_training.jpg','jpg');
extraface1=rgb2gray(extraface1);
extraface1=imresize(extraface1,[N N] );
figure(5),imshow(extraface1,'Initialmagnification','fit');;title('extraface_training')

%% m_gr.jpg
extraface2 = imread('john_training.jpg','jpg');
extraface2=rgb2gray(extraface2);
extraface2=imresize(extraface2,[N N] );
figure(6),imshow(extraface2,'Initialmagnification','fit');;title('john_training')


%% store
st.names = {'mic','shelly','linoy','libi','exface','john'};
st.data{1} = mic1;
st.data{2} = shelly1;
st.data{3} = linoy1;
st.data{4} = libi1;
...
testFaceMic = imread('test1.jpg','jpg');
...
testFaceLinoy = imread('test2.jpg','jpg');
...
%% libi3.jpg
testFaceLibi = imread('test3.jpg','jpg');
